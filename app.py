...
def init_google_sheets():
    ...
    try:
        ...
        try:
            ...
            print(f"Error during Google Sheets authorization: {str(e)}")
            return None
            
    except Exception as e:
        print(f"Error with Google Sheets initialization: {str(e)}")
        return None
...
def admin():
    ...
    try:
        ...
        if client is None:
            ...
            sheet_routes, sheet_links = init_spreadsheet(client)
            if sheet_routes is None or sheet_links is None:
                print("Failed to initialize spreadsheet")
                return render_template('admin.html', routes=[], error="שגיאה בטעינת הגיליונות")
